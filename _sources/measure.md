...
```{prf:definition}
:label: def-filter
A non-empty family $\mathcal{F} \subseteq \Pow(X)$ of subsets of a given set $X$ is an **filter** if 
1. $A \in \mathcal{F}$ and $B \supseteq A$ implies $B \in \mathcal{F}$,
2. $A, B \in \mathcal{F}$ for all $n$ implies $A\cap B \in\mathcal{F}$.
```
