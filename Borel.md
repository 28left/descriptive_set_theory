...
- *countable intersections*:   $\bigcap A_n = \Co{\bigcup_n \Co{A_n}}$.
- *differences*:   $A \setminus B = A \cap \Co{B}$.
- *Symmetric differences*:   $A \bigtriangleup B = (A \cap \Co{B}) \cup (\Co{A} \cap B)$.
...
The first statement was proved in Lemma 1. The second statement follows by passing to complements: If $U$ is open, 
\begin{equation*}
	U = \Co{(\Co{U})} = \Co{\bigcap U_n} = \bigcup \Co{U_n},
\end{equation*}
where the $U_n$ are open.
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
is a function that is continuous at every irrational and discontinuous at every rational number. How about the other way around - **discontinous at exactly the irrationals?** As noted above, the rationals are a $\bSigma^0_2$ set that is not $\bPi^0_2$. Hence **such a function cannot exist**.
...
It follows by uniform convergence that if each $f_n$ is continuous at $x$, $f$ is continuous at $x$, too. Hence $f$ is continuous on $A$.

Now assume $x \notin A$. Then there exists $n$ such that $x \in G_n \setminus G_{n+1}$. Hence 
\begin{equation*}
	f_0(x) = \dots = f_n(x) = 0.
\end{equation*}
Again, we distinguish two cases. 

First, assume $x \not\in \Cl{G_{n+1}}$. Then there exists $\delta > 0$ such that $U_\delta(x) \subseteq \Co{G_{n+1}}$. This also implies  $U_\delta(x) \subseteq \Co{G_{k}}$ for any $k \geq n+1$. Besides, since $G_n$ is open, we can chose ${}\delta$ sufficiently small so that $U_\delta(x) \subseteq G_n$. This implies $f_k(y) = 0 \; \text{ for all }\; k \leq n, \;  y \in U_\delta(x)$.

For $y \in \Co{D} \cap U_\delta(x)$ we have $f_k(y) = 1$ for all $k \geq n+1$, and hence $f(y) = \sum_{k > n} b_k f_k(y) > 0$. On the other hand, if $y \in D \cap U_\delta(x)$, then $f_k(y) = 0$ for all $k \geq n+1$, and also $f_0(y) = \dots = f_n(y) = 0$, since $y \in G_n$, and thus $f(y) = 0$. Hence there are points arbitrarily close to $x$ whose $f$-values differ by a constant lower bound, which implies $f$ is not continuous in $x$[^y_exists].
...
[^y_exists]: Strictly speaking, for this proof to work we need to know that non-empty open sets in perfect Polish spaces are uncountable. We will prove this in the next chapter.
