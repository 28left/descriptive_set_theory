...
If $D = \Op{Dom}(F) \neq \Ord$, then we would have $D = \alpha$ for some ordinal ${}\alpha$. In particular $D$ is a set and therefore $F = f$ is a set, for some tame $f$. This $f$ could be extended to a tame $h = f \cup \{(\alpha,G(\alpha,f \Rest{\alpha}))\}$, contradiction.
```
...
Suppose $R$ is a well-founded relation on a class $A$. For every non-empty class $B \subseteq A$, there exists $a \in B$ such that
$$
    \forall y \in B \;  \neg y R a
$$
```
