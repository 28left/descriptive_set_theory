...
In the example above, the set $M$ is not transitive, which allowed it to "hide" its elements. It turns out that if we require our model to be transitive, the truth of simple formulas cannot vary between the "inside" and the "outside" perspective.
