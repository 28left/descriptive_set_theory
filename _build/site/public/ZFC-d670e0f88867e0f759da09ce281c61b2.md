...
We have seen we cannot use full comprehension for sets. In its place we introduce the scheme of Separation. Let $\varphi(x, w_1, \dots, w_n, z)$ be a $\mathcal{L}_\in$-formula in which $y$ does not occur as a free variable.

:::{prf:axiom} Separation$_{{}\varphi}$
:nonumber: true

 $\qquad  \forall w_1, \dots, w_n \forall a \exists y \forall x (x \in y \leftrightarrow x \in a \wedge \varphi(x,w_1, \dots, w_n, a))$
:::
