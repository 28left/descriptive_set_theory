...
A (**reflexive** or **non-strict**) **teilweise Ordnung** on a set $A$ is a binary relation $\leq$ on $A$ such that for all $a,b,c \in A$,
- (a) $a\leq a$ &emsp; (reflexive)
- (b) $a \leq b$ and $b \leq a$ implies $a = b$ &emsp; (anti-symmetric)
- (c) $a \leq b$ and $b \leq c$ implies $a \leq c$ &emsp; (transitive).
A **linear** (or **total**) **order** additionally satisfies
- (d) $a \leq b$ or $b \leq a$  &emsp; (connected).
```
