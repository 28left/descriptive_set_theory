...
It is straightforward to verify that $(\alpha_0, \beta_0)$ is the smallest element of $A$ with respect to $<_g$. Note that
...
Now assume it does not hold for all larger cardinals. Since every infinite cardinal is of the form $\kappa = \aleph_\alpha$, choose ${}\alpha > 0 $ minimal such that $F(\aleph_\alpha \times \aleph_\alpha) \neq \aleph_\alpha$. 
